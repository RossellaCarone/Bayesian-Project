...
telesca_model <- function(y, niter = 100, nburn = 100, n_knots_h = 10, n_knots_m = 30,m_c0=0, sigma_c0=1, a_c=0, b_c=1, m_a0=1, sigma_a0=1, a_a=1, b_a=1,
                          a_eps = .1, b_eps = .1, a_lambda = .1, b_lambda = .1, r_phi = rep(1,n_knots_h + 3), f_phi = rep(1,n_knots_h + 3), coeff_var=0.5
){
  ...
  tune <- .005
  accepts = matrix(0, nrow=n_patients, ncol=q-1)
  ...
  csi_initial <- numeric(q)
  for (j in 1:(q-1)){
    csi_initial[j+1]=rgamma(n=1,r_phi[j], f_phi[j])
  }
  
  somma=sum(csi_initial)
  phi_initial=numeric(q)
  phi_initial=cumsum(csi_initial)/somma
  
  csi<-lapply(1:n_patients, function(i) csi_initial)

  phi <- lapply(1:n_patients, function(i) phi_initial)
  
  # tensor with all phi_i
  phi_save = array(rep(NA, nrun *q*n_patients), dim=c(nrun , q , n_patients))
  for (i in 1:n_patients){
    phi_save[1, ,i] = phi_initial
  }
  ...
  for(iter in 2:nrun){
    ...
    for (i in 1:n_patients){
      ...
      for (b in 2:q){
        #phi,csi, beta, sigma_eps, a_i, c_i,r,f, p, B_h, y_i, n_obs_i

        pi_phi_old = pi_phi(phi_old ,csi_old[b], beta_save[iter-1,],  sigma_eps_save[iter-1], a_save[iter-1, i],
                            c_save[iter-1,i], r_phi[b-1],f_phi[b-1], p,  Bh[[i]],
                            as.numeric(y_list[[i]]), n_obs[i])
        
       # coeff_var = 0.5
        r_prop= 1/coeff_var^2
        f_prop= r_prop/csi_old[b]
        
        csi_new[b]=rgamma(n=1,r_prop,f_prop) 
        somma_new=sum(csi_new)
        phi_new<-cumsum(csi_new)/somma_new

        pi_phi_new = pi_phi(phi_new,csi_new[b], beta_save[iter-1,], sigma_eps_save[iter-1],a_save[iter-1, i],
                            c_save[iter-1,i], r_phi[b-1],f_phi[b-1],p,  Bh[[i]],
                            as.numeric(y_list[[i]]), n_obs[i])
        
        quoziente=(pi_phi_new-pi_phi_old)+(dgamma(csi_old[b],r_prop,f_prop,log=T)-dgamma(csi_new[b],r_prop,f_prop,log=T)) #da cambiare i parametri
        ...
        if(u<exp(alpha)){
          #accept
          #if(abs(csi_new[b]-csi_old[b])>1e-6)
          #if(max(abs(csi_new-csi_old))>1e-6)
...
n_knots_h=3
n_knots_m=20
r_phi = rep(4, n_knots_h+3) 
f_phi = rep(0.8, n_knots_h+3) 
coeff_var = 0.3
#graphics.off()
# x11()
# invgamma = rgamma(n=1000, r_phi[1], f_phi[1])
# hist(invgamma)
# median(invgamma)

#y= read.csv('Data_Undersampled_healthy.csv') #qua caricate il dataset con l'undersampling
y = read.csv('Data_Undersampled_surgery72.csv')
...
telesca_output = telesca_model(y=y, niter = niter, nburn = nburn, n_knots_h = n_knots_h, 
                             ...
                             r_phi = r_phi, f_phi=f_phi, coeff_var = coeff_var)
end.time <- Sys.time()
...
# 2.8 minuti per 2000 iterazioni con 10 pazienti, 8 nodi h, 40 nodi m, con undersampling (1 ogni 4)
...
x11()
matplot(beta_output[(nburn+1):(niter+nburn),],type='l',
...
x11()
par(mfrow=c(2,1))
matplot(phi_output[(nburn+1):(niter+nburn),,1],type='l',xlab = 'Iteration', main='phi of patient 1')
matplot(phi_output[(nburn+1):(niter+nburn),,6],type='l',xlab = 'Iteration', main='phi of patient 6')


x11()
matplot(sigma_eps_output[(nburn+1):(niter+nburn)], type='l',xlab = 'Iteration', main='sigma^2_eps')
...
x11()
matplot(sigma_eps_output, type='l',xlab = 'Iteration', main='sigma^2_eps')
...
x11()
plot(time_grid, h_i[,1], type='l', xlab = 't', ylab='h_i(t)', col=col_pat[1], cex.lab=1.5, cex.axis=1.2)
for(i in 2:n_patients){
  points(time_grid, h_i[,i], type='l', col=col_pat[i])
}
...
# batchSE(prova, batchSize = 50) # da capire
s=summary(a_mcmc)
mcse = s$statistics[,4] #MC standard errors
round(median(mcse), digits=4)
# ESS
ess_a = effectiveSize(a_mcmc)
median(ess_a)
...
#### Correlation phi ####
pat = phi_output[-c(1:nburn),-c(1,7),3]
cor(pat)

x11()
matplot(pat,type='l',xlab = 'Iteration', main='phi of patient i')
